...
func NewLokiClient(cfg *Config, logger log.Logger, metrics *client.Metrics) (*loki, error) {
	internalCfg, err := parseConfig(cfg)
	if err != nil {
		return nil, err
	}
	client, err := newClient(internalCfg, logger, metrics)
	if err != nil {
		return nil, err
	}
	return &loki{
...
// SendRecord send records to loki as an entry.
...
func newLogger(logLevel string) log.Logger {
	...
	logger = level.NewFilter(logger, LogFilter(logLevel))
